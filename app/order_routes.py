...
async def list_all_order(current_user: User=Depends(get_current_user),
                         db :Session= Depends(get_db)):
...
async def get_order_by_id(id:int, current_user:User = Depends(get_current_user),db:Session =Depends(get_db)):
...
async def get_specific_order(id:int,current_user: User = Depends(get_current_user),db:Session = Depends(get_db)):
    order = db.query(Order).filter(Order.id==id).first()
    
    if order is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="this order is not found")
    
    if order.user_id!=current_user.id and not current_user.is_staff:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail='you are not the user')
    return jsonable_encoder(order)

@order_router.put('/update/order/{id}')
async def update_specific_order(id:int, order: OrderModel,current_user:User = Depends(get_current_user),db:Session = Depends(get_db)):
...
async def delete_order(
    ...
    if order_to_delete.user_id == current_user.id or  current_user.is_staff:
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="you cannot delete this order")
